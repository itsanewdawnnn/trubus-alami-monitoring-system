...
function remote_management_definitions(): array
{
    return [
        ...
        'outlet_min_visits_per_day' => [
            'label' => 'Minimum Outlet Visits (per day)',
            'hint' => 'Minimum number of distinct outlets a Member should visit per day. Used by the Visit Report under the Outlet menu; does not reject a visit falling short of this target.',
            'default' => 30,
            'min' => 1,
            'max' => 200,
        ],
        'outlet_radius_meters' => [
            'label' => 'Outlet Radius (meters)',
            'hint' => 'Maximum distance from the outlet location for a GPS fix to be considered inside that outlet, evaluated by backend/api.php on every fix received while tracking is active.',
            'default' => 100,
            'min' => 10,
            'max' => 1000,
        ],
        'outlet_min_dwell_seconds' => [
            'label' => 'Minimum Dwell Duration In Radius (seconds)',
            'hint' => 'Minimum time a Member must remain inside the outlet radius before a visit is considered confirmed. 0 (default) means a visit is recorded immediately once a single GPS fix enters the radius.',
            'default' => 0,
            'min' => 0,
            'max' => 3600,
        ],
