...
(function () {
    ...
    function populateDetailCard(member, lookupLandmark) {
        ...
        // Advisory-only mock-location warning (see ajax/live_tracking.php's
        // own comment on is_mock_location/gnss_satellites_used) -- purely
        // informational for the Admin to judge, never hides/blocks anything
        // else on this card. gnss_satellites_used === 0 means the fix's GNSS
        // status callback reported zero satellites actually used, which is a
        // corroborating signal alongside isMock, not a standalone trigger on
        // its own noisy edge cases (e.g. briefly indoors) -- so it's only
        // treated as a flag together with a fresh fix, same as isMock.
        var mockBadge = document.getElementById('liveDetailMockBadge');
        var looksMocked = member.status === 'active' &&
            (member.is_mock_location === true || member.gnss_satellites_used === 0);
        mockBadge.hidden = !looksMocked;
