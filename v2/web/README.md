...
|-------------|----------------|
| `pages/`    | One file per screen (dashboard, members, live tracking, member history, member log, OTA update, remote management, outlet). Each is both the controller and the view -- there's no separate template layer, which is a deliberate choice for an app this small. |
...
**Mock-location trust signals**: both `ajax/live_tracking.php` and
`ajax/history.php` also select `is_mock_location`/`gnss_satellites_used`
(present on both `tams_live_tracking_current` and
`tams_member_history_locations`) and pass them straight through in their
JSON response. `assets/js/live_tracking.js`'s `populateDetailCard()` and
`assets/js/history.js`'s `showPointDetail()` show a passive "Possible Mock
GPS" `badge--warning` on the respective detail card when
`is_mock_location === true` or `gnss_satellites_used === 0` -- purely
informational, never hides a member/point or alters any other computation.
Both fields are advisory-only end to end (see
[`android/README.md`](../android/README.md#mock-location-detection) for the
Android-side detection this displays) -- `NULL` means "unknown", never
"confirmed genuine", and neither this page nor `backend/api.php` ever uses
either field to reject or hide a fix.
...
An outlet is a physical location a Member (sales) is expected to visit.
Members register outlets from the Android app, subject to Admin approval;
Admins can also create outlets directly from this page (auto-approved) and
assign an outlet to a Member. Visit detection is
geofencing, computed entirely server-side from the Member's own ordinary
GPS tracking -- there is no separate "check in" action anywhere in this
feature. See [`android/README.md`](../android/README.md#outlet-management)
for the client side.

**Four tables** (`database/schema.sql`) back this feature:
...
**Approval workflow.** A Member's own submission (`POST /outlet/create` on
the Backend API) always starts PENDING. An Admin creating an outlet
directly (`ajax/outlet_create.php`) is auto-approved immediately -- an
Admin-created outlet is authoritative the moment it exists, no review step
needed. `ajax/outlet_approve.php`/`ajax/outlet_reject.php` decide, fresh
from a locked read of the outlet row (never trusted from the caller),
whether they're approving/rejecting the outlet itself (PENDING/REJECTED
status) or a pending edit request against an already-APPROVED outlet --
matching the same dual meaning the Member-facing `/outlet/update` route
...
`tams_outlets` (a REJECTED outlet also resets to PENDING for a fresh
review). Once APPROVED, `tams_outlets`' own name/address/latitude/longitude
-- the exact columns the geofencing hook below reads -- stays untouched for
as long as any proposed edit sits unreviewed; the edit is instead proposed
as a full snapshot in `tams_outlet_edit_requests`, and only overwrites
`tams_outlets` once an Admin approves it. A Member refining an
already-open edit request supersedes it in place rather than opening a
competing second one (that table is workflow state, not a ledger -- safe to
update in place, unlike `tams_outlet_visits`). An Admin's own direct edit
(`ajax/outlet_update.php`) never goes through this queue at all -- it *is*
the authoritative decision, with no higher approval needed -- and
supersedes (marks REJECTED, never deletes) any still-open Member edit
request against that outlet in the same transaction, so a later Approve
click can never blindly apply a now-stale proposal over the Admin's own
edit.

**Surfacing a rejected edit back to the Member.** Rejecting an edit request
(above) never touches `tams_outlets.rejection_reason` -- that column only
ever describes the *outlet's own* PENDING->REJECTED transition, never an
edit request's. Found during Final Validation: without a separate signal,
a Member whose proposed edit got rejected saw `has_pending_edit` quietly
flip back to `false` with no visible reason anywhere. The Backend API's
`GET /outlet/list` now also returns `last_edit_rejection_reason` -- a
correlated subquery picking this outlet's single most recent edit request
(any status) and returning its `rejection_reason` only when that latest one
is REJECTED, so a later APPROVED resubmission correctly stops showing an
older, superseded rejection. The Android app (`OutletCard`) shows this as
"Last edit rejected: ..." whenever the outlet is APPROVED with no currently
open edit request. The Web Admin side needs no equivalent -- the Admin is
the one who wrote the rejection reason in the first place.

**Geofencing / visit detection.** Computed inside `backend/api.php`'s
existing `POST /location/update` transaction, right after that route's own
history-row insert -- not a new endpoint, matching this project's
single-write-path philosophy for anything derived from a GPS fix (see the
"Trip History data model" note above). For each fix that already passed GPS sanity filtering,
the route reads every APPROVED/non-deleted/non-merged outlet assigned to
the Member (`tams_outlets.member_id`, a plain unlocked `SELECT` -- outlet
assignment is private per Member, so unlike Force Location's `tams_users`
toggle, no concurrent writer ever contends for this specific read; a Member
may own several outlets even though each outlet has exactly one owner, see
"Ownership vs. assignment" below), computes
haversine distance to each, and confirms a visit once the Member is within
`outlet_radius_meters` -- immediately if `outlet_min_dwell_seconds` is `0`
(the default), or once that many seconds have elapsed continuously inside
the radius otherwise, tracked in `tams_outlet_dwell_state` and cleared the
moment the Member leaves the radius, the visit confirms, or the Member
sends a Stop Location signal (closing a gap where resuming tracking long
after an early Stop would otherwise measure elapsed dwell time across a
period the Member's real position was unknown). Both values are
Remote Management-configurable (see below), read with the same
default/clamp fallback every other Remote Management value uses --
`outlet_radius_meters` defaults to 100m (10-1000m range),
`outlet_min_dwell_seconds` defaults to 0 (0-3600s range). A third value,
`outlet_min_visits_per_day` (default 30), gates nothing server-side -- it's
a reporting target only, read by the Visit Report tab (see "Visit Report"
below).

**Ledger immutability.** `tams_outlet_visits` is append-only and must never
be updated or reassigned after being written, including by
`ajax/outlet_merge.php`. Merging soft-deletes the source outlet and points
it at the survivor via `merged_into_outlet_id`; past `tams_outlet_visits`
rows keep referencing the original (now-merged-away) outlet id forever, and
a report resolves "current" identity by following that pointer instead --
exactly the same principle `tams_member_history_locations` already follows
for location fixes. The survivor's own `member_id` is likewise left
completely unchanged by a merge, consistent with its name/address/
coordinates also staying untouched -- the source's assignment is simply
discarded along with the rest of its now-soft-deleted row (there is nothing
to union under a one-to-one relationship).

**Visit Report.** `ajax/outlet_visit_report.php` backs the Outlet page's
"Visit Report" tab -- for a single calendar day (default today, server/WIB
time; a Y-m-d `date` query param picks a different one), one row per active
Member **who visited at least one outlet that day** (an `INNER JOIN` from
`tams_users` to `tams_outlet_visits`, not a `LEFT JOIN` -- a Member who
visited nothing contributes no matching row and is correctly absent; the
report exists to show who actually visited something and what, not to
enumerate every active Member regardless of activity), showing their
distinct-outlet visit count and whether it meets `outlet_min_visits_per_day`
(green "Target met" badge) or falls short (orange "Below target" badge).
Reads `tams_outlet_visits` directly, keyed by `(member_id, visited_date)`,
with no join back to `tams_outlets` at all -- a visit to an outlet since
soft-deleted or merged away still counts and still shows under its
`outlet_name_snapshot`, matching the ledger-immutability principle above.
The visited-outlets name list (table cell) and the per-outlet detail array
(`visit_details`: name, `confirmed_at`, `dwell_seconds` -- backing the
row-click popup below) are both built from the same second query keyed by
`member_id`, read once and merged into both shapes in PHP -- the same
avoid-`GROUP_CONCAT` idiom `ajax/outlet_list.php` used for its own
(now-removed) per-outlet Member list, rather than risking `GROUP_CONCAT`'s
default 1024-byte truncation ceiling, and never a second query for the
detail data. Each row is clickable (`assets/js/outlet.js`), opening a
read-only popup (name, username, total visits, status, and every visited
outlet with its confirmed time and dwell duration if any) built entirely
from this same already-fetched response -- no additional request per click,
and no edit/delete action, since this report is observation-only.

**Locking / transaction boundaries.** Every outlet mutation that changes
more than one row runs inside an explicit transaction with a `SELECT ...
FOR UPDATE` row lock taken first, so a concurrent duplicate request (a
network retry racing itself, or an Admin action landing mid-Member-edit)
waits instead of interleaving: `ajax/outlet_approve.php`/
`ajax/outlet_reject.php` lock the outlet row (and, if applicable, the
`tams_outlet_edit_requests` row); `ajax/outlet_update.php` locks the outlet
row before superseding any open edit request; the Backend API's own
`/outlet/update` locks the outlet row, scoped to `created_by_user_id` (see
"Ownership" below), before deciding which branch of the edit workflow
applies. `ajax/outlet_merge.php` locks **both** outlet rows, always in
ascending `id` order regardless of which one is labeled source/target --
...
Backend API -- both `/outlet/update` and `/outlet/delete` scope their
`WHERE` to it. `tams_outlets.member_id` (who the outlet counts toward, and
who the geofencing hook above fires for) is a separate relationship an
Admin can change independently -- one-to-one from the outlet's side (exactly
one Member per outlet; a Member may own many outlets), enforced by a plain
`NOT NULL` `FOREIGN KEY` column rather than a join table. A Member assigned
to an Admin-created outlet can see it and have visits recorded against it,
but can never edit or delete it -- only the creator (or an Admin, with no
ownership restriction at all) can. Reassigning an outlet
(`ajax/outlet_update.php`) is a single-column `UPDATE`; this doubles as the
...
allowed while `status` is PENDING or REJECTED -- an outlet in either state
structurally cannot yet have any `tams_outlet_visits` rows (visits only
ever confirm against APPROVED outlets), so nothing of value is destroyed.
`ajax/outlet_delete.php` (Admin, any status) is always a soft delete
(`deleted_at`) instead, since an APPROVED outlet may already have visit
history that must survive it being removed from every active list.

**Design decisions:**

- **Why one-to-one (`tams_outlets.member_id`) instead of a many-to-many join
  table** -- an outlet is meant to represent one Member's own territory;
  every query, filter, and report that cares "which Member owns this
  outlet" gets a single indexed column to read instead of joining and
  de-duplicating a separate assignment table, and reassignment becomes one
  atomic `UPDATE` instead of a delete-then-reinsert pair.
- **Why outlets are private per Member for the geofencing read** -- unlike
  Force Location's `tams_users.force_tracking_hours` (a value an Admin can
  change concurrently with an in-flight fix, requiring a lock to close a
  real race), an outlet's assignment set changing mid-fix is a low-impact,
  self-correcting race (the next fix picks up the new assignment), so the
  read is a plain unlocked `SELECT` -- a Member with no assigned outlets
  pays for one cheap indexed lookup that returns zero rows, never a table
  scan.
- **Why `outlet_display_name()`'s "NAME: OUTLET" formatting is derived at
  read time, never stored** -- persisting it would go stale the moment the
  creator renames their own account; deriving it from `tams_outlets.name` +
  a join to `tams_users.name` costs one join, not a background job to keep
  two copies in sync. Admin-created outlets show the literal word "ADMIN"
  instead of an individual Admin's name, by product decision -- an
  Admin-assigned outlet is company-issued, not personally attributed.
...
`tams_outlet_visits`, `tams_outlet_dwell_state`, and friends).
...
- **Confirmed efficient, no changes needed:** every endpoint in
  `backend/api.php` and `ajax/*.php` issues a small, fixed (or Member-
  assigned-outlet-count-bounded) number of queries per request regardless of
  overall data volume -- most routes 1-3, `/location/update` up to 5 in its
  base branch (Force Override check, previous-fix lookup, the position
  upsert, the Member Version Monitoring device-info update, and the
  history/snapshot insert), plus the Outlet Management geofencing hook's own
  small addition when the Member has assigned outlets and the fix passed GPS
  sanity filtering: one indexed lookup of the Member's own APPROVED
  outlets (`tams_outlets.member_id`, typically 0-5 rows for a real Member,
  never a table scan), one more if dwell-time mode is
  enabled (`tams_outlet_dwell_state`, keyed by the same Member), and at most
  one write per candidate outlet (a visit confirm and/or a dwell-state
  insert/delete) -- bounded by that Member's own small assigned-outlet
  count, not by `tams_outlets`' overall size, and every one of those writes
  is `INSERT IGNORE`/keyed by a primary or unique index, so this stays cheap
  regardless of how large `tams_outlet_visits` grows over time. No query in
  this hook lacks an index for its `WHERE`/`JOIN` columns, and none runs a
  per-row loop that itself issues an unbounded number of further queries
  (the per-candidate loop's own queries are 1:1 with that Member's own
  assigned-outlet count, which in practice is small and Admin-controlled).
  Elsewhere, no query lacks
