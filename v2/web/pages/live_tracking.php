...
<div class="tab-panel" id="panelMap" hidden>
    <div class="map-container">
        ...
        <div class="map-detail-card" id="liveDetailCard" hidden>
            ...
            <!-- Advisory-only mock-location warning (see schema.sql's
                 "Mock-location trust signals" comment) -- shown, never
                 blocking, purely for manual review. On its own line (not
                 crammed into the row above) since the card is only 280px
                 wide and that row already has no wrap. Hidden by default;
                 assets/js/live_tracking.js's populateDetailCard() reveals it
                 only when the current fix is flagged. -->
            <span class="badge badge--warning" id="liveDetailMockBadge" hidden>Possible Mock GPS</span>
