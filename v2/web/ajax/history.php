...
$stmt = $pdo->prepare("
    SELECT latitude, longitude, accuracy, speed, is_moving, recorded_at,
           is_mock_location, gnss_satellites_used
...
$points = array_map(static function (array $p): array {
    return [
        ...
        // "Mock-location trust signals" comment. Informational only, never
        // affects distance/duration/point-count above. null means
        // "unknown" (older app version, or a fix predating this feature).
        'is_mock_location' => $p['is_mock_location'] !== null ? ((int) $p['is_mock_location']) === 1 : null,
        'gnss_satellites_used' => $p['gnss_satellites_used'] !== null ? (int) $p['gnss_satellites_used'] : null,
    ];
}, $rawPoints);
